 ...
 * @param {String} user.username - The username (phone)
 * @param {Object} client - The client object
 * @param {String} client._id - The client id
 */

async function generateTokens(userToken, client) {
  ...
  await RefreshToken.findOneAndRemove({ user: userToken._id });
...
server.exchange(oauth2orize.exchange.password(async (client, username, password, scope) => {
  try {
    ...
    return await generateTokens(userToken, client);
  } catch (err) {
    log.error(err);
  }
  return false;
